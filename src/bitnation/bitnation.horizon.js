...
(function (Bitnation, $) {
     ...
     */
    var Address = function(accountRS) {
        var address = {};

        /**
         * Verify that the Reed Solomon address is valid
         */
        address.verifyRS = function () {
            return (
                this.accountRS !== undefined &&
                this.accountRS.substring(0, 4) === 'NHZ-' &&
                this.accountRS.length === 24
            );
        }

        /**
         * Get an account ID from a secret phrase
         *
         * From NRS
         */
        address.getAccountId = function(secretPhrase) {
            return this.getAccountIdFromPublicKey(
                this.generatePublicKey(secretPhrase)
            );
        };

        if (accountRS === undefined) {
            // If the accountRS hasn't been supplied, assume this is a brand
            // new account & generate a secret phrase for it
            var pwg = new _PassPhraseGenerator();
            pwg.generatePassPhrase()
            .done(function (secret) {
                address.secretPhrase = secret;
            })
            .fail(function (err) {
                throw err;
            });
        } else {
            // Otherwise just set the address locally
            address.accountRS = accountRS;
        }

        // Parse the address if we can - if it fails, return false
        if ((accountRS !== undefined && address.verifyRS()) || accountRS === undefined) {
            return address;
        } else {
            return false;
        }
